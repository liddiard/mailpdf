...
const DEFAULT_REQUIRED_FIELDS = new Set(['name', 'line1', 'city', 'state', 'zip'])
...
const App = ({
  ...
  const [file, setFile] = useState({ // populated by upload api response
    filename: '',
    uid: '',
    url: '',
    numPages: 0
  ...
  const [fromAddress, setFromAddress] = useState(() => makeAddress(requiredFields))
  ...
  const missingFields = useCallback(address => {
    return new Set(Array.from(requiredFields).filter(field => {
  ...
  const updateAddress = useCallback((isFrom, field) => {
  ...
  const verifyAddress = useCallback(async (isFrom, address) => {
    ...
    try {
      ...
      console.error(err)
      setter(prev => ({ ...prev, error: 'Unable to verify this address. Please try again.', dirty: false }))
    }
  }, [])
  ...
  return (
    <main>
      ...
      <Header costs={costs} file={file}
              fileUploadHasBegun={beginFileUpload}
              updateFile={updateFile}
              actionable={!sentSuccessfully} />
      <Envelope fileUploadHasBegun={fileUploadHasBegun}
                updateAddress={updateAddress}
                fromFields={fromAddress}
                toFields={toAddress}
                sentSuccessfully={sentSuccessfully} />
      <Send costs={costs} file={file}
            options={options} updateOptions={updateOptions}
      ...
      <footer>
        <ul>
          <li><a href="/pages/refund-policy" target="_blank" rel="noreferrer">Refund Policy</a></li>
          <li><a href="/pages/terms" target="_blank" rel="noreferrer">Terms of Service</a></li>
          <li><a href="/pages/privacy" target="_blank" rel="noreferrer">Privacy Policy</a></li>
        </ul>
        <p>Powered by <a href="https://lob.com/" target="_blank" rel="noreferrer">Lob</a> and <a href="https://stripe.com/" target="_blank" rel="noreferrer">Stripe</a></p>
      </footer>
    </main>
  )
}
