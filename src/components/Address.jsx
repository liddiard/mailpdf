...
const Address = ({ fileUploadHasBegun, from, fields, updateAddress }) => {
  ...
  if (fields.error) {
    ...
    verificationStatus = (
      <i className="verification-status fa fa-times" aria-hidden="true"></i>
    );
  }
  if (fields.dirty) {
    ...
    verificationStatus = (
      <i
        className="verification-status fa fa-circle-o-notch fa-spin"
        aria-hidden="true"
      ></i>
    );
  }
  // strict equality comparison because error can also be `undefined`
    ...
    verificationStatus = (
      <i className="verification-status fa fa-check" aria-hidden="true"></i>
    );
  }

  const className = `address ${title} ${error ? "error" : ""}`;
  const arrow = (
    <i className={`fa fa-arrow-right ${title}`} aria-hidden="true" />
  );
